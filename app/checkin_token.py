...
def generate_checkin_token(
    ...
    return jwt.encode(claims, settings.checkin_token_secret, algorithm=_ALGORITHM)


def verify_checkin_token(token: str) -> UUID:
    try:
        claims = jwt.decode(token, settings.checkin_token_secret, algorithms=[_ALGORITHM])
